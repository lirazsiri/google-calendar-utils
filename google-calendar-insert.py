...
def calendar_get_all_events(cal, calendar_id):
    all_events = list(cal.iter_events(calendar_id, singleEvents=True,
                                                   showDeleted=False,
                                                   orderBy='startTime'))
...
def main():
    ...
    all_events = calendar_get_all_events(cal, calendar_id)

    inserted_events = filter_duplicates(all_events, inserted_events)
    if options.simulate:
        for event in inserted_events:
            print simplejson.dumps(event, indent=True)
    else:
        for event in cal.insert_events(calendar_id, filter_duplicates(all_events, inserted_events)):
            print simplejson.dumps(event, indent=True)
